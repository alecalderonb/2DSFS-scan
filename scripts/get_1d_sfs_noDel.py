...
with open(output_file, "w") as out:
    out.write("region\tgeneration\tfreq_p1\tcount\tdensity\n")
    ...
    for region in sfs:
        for gen in sfs[region]:
            ...
            for i in range(20):
                if i > 0: #and sfs[region][gen][i][j] > 0:
                    count = sfs[region][gen][i]
                    density = sfs[region][gen][i] / total
                    out.write(
                        f"{region}\t{gen}\t{i}\t{count}\t{density}\n"
                    )    