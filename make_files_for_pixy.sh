...
gunzip -c slim_sims/LHU_sims/simulations_LHU/vcfs_dadiparams/migRate.${migRate}.${gen}.vcf.gz  | awk '$1 ~ /^#/ {print $0;next} {print $0 | "sort -k1,1 -k2,2n"}' > slim_vcfs/migRate.${migRate}.${gen}.vcf
bgzip  -f slim_vcfs/migRate.${migRate}.${gen}.vcf

# run our likelihood calculatiopn on this sorted vcf
python scripts/run_analyses.py False slim_vcfs/migRate.${migRate}.${gen}.vcf.gz > sim_likelihood_calcs/migRate.${migRate}.${gen}.temp.ll

# get rid of annoying warning message
cat sim_likelihood_calcs/migRate.${migRate}.${gen}.temp.ll | awk '{if(NR> 2) print}' > sim_likelihood_calcs/migRate.${migRate}.${gen}.ll

# index sorted vcf with tabix
tabix -f -p vcf slim_vcfs/migRate.${migRate}.${gen}.vcf.gz
